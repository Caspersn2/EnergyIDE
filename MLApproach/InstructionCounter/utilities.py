...
sqstring = r'\'[^\']\''
identifier = fr'{id} | {sqstring}'
dotted_name = fr'{identifier}(?:.{identifier})*'
class_type = r'(?:class\s)((?:\S+\s?)+)'
method_name = fr'(?:{generic_type}|{primitive_type}|{library_returntype}|class {dotted_name})\s({variable_name})\s\((.|\s)*?\)'
